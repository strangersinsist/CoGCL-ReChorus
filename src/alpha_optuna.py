...
def suggest_hyperparameters(trial: optuna.Trial) -> Dict:
    ...
    Only searches for code_mix_alpha, all other hyperparameters use fixed default values.
    """
    # Fixed hyperparameters (from user-specified optimal values)
    params = {
        # Fixed core loss weights
        'vq_loss_weight': 0.008,
        'cl_weight': 0.03,
        'sim_cl_weight': 0.5,
        
        # Fixed graph augmentation parameters
        ...
        # Fixed contrastive learning temperature
        'cl_tau': 0.47,
        
        # ONLY search for code_mix_alpha (QER fusion coefficient)
        # Range: 0 to 0.3, log scale
        'code_mix_alpha': trial.suggest_float('code_mix_alpha', 1e-6, 0.3, log=True),
    }
    return params
...
def create_objective(base_args, corpus):
    """
    Create the Optuna objective function.
    Only searches for code_mix_alpha hyperparameter.
    """
    
    def objective(trial: optuna.Trial) -> float:
        # Get hyperparameters from Optuna (only code_mix_alpha is searched)
        ...
        # Apply searched hyperparameter
        args.code_mix_alpha = hp['code_mix_alpha']
        ...
        logging.info(f"\n{'='*50}")
        logging.info(f"Trial {trial.number}: code_mix_alpha={hp['code_mix_alpha']:.6f}")
        logging.info(f"{'='*50}")
        ...
        if args.code_mix_alpha > 0:
            ...
            model_class = CoGCL
            logging.info(f"Using CoGCL_new.py (QER enabled, code_mix_alpha={args.code_mix_alpha:.6f})")
        else:
...
def main():
    ...
    # 注意：CoGCL 模型的动态导入已移到 objective 函数中
    # 因为 code_mix_alpha 在每个 trial 中会变化
    
    # Load model class for reader
    ...
    # Create objective function (only searches code_mix_alpha)
    objective = create_objective(args, corpus)
